...
export function injectQrCodes(
  ...
  content = content.replace(/\{\{qrCode\}\}/g, () => {
    ...
    return `<draw:frame draw:name="QR-${qr.enrollmentId}" text:anchor-type="as-char" svg:width="2cm" svg:height="2cm" draw:z-index="0"><draw:image xlink:href="${fileName}" xlink:type="simple" xlink:show="embed" xlink:actuate="onLoad"/></draw:frame>`;
  });
